...
/* Standalone build: avoid pulling in NextUI's headers. */
#ifndef SDCARD_PATH
...
static int parse_file(const char *path) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        /* Decode \n / \t / \\ escapes so multi-line dialog strings can
         * sit on a single .lang line. */
        size_t r = 0, w = 0;
        while (r < vlen) {
            if (v[r] == '\\' && r + 1 < vlen) {
                char nxt = v[r + 1];
...
void I18N_init_from_minuisettings(void) {
    ...
    if (f) {
        char line[128];
        while (fgets(line, sizeof(line), f)) {
            if (sscanf(line, "language=%7s", lang) == 1) break;
        }
        fclose(f);
    }
    I18N_init(lang);
